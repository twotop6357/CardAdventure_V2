...
1. 전사 카드 5종 효과를 실제 전투 루프에 연결한다. 특히 `방패치기`, `분노`, `도발`은 카드별 특수 로직이 필요하다.
2. 상태이상 턴 처리 구현: 독, 약화, 취약, 강화, 재생.
3. 적 테스트 데이터 1~2종과 전투 테스트 씬 구성.
4. 기본 배틀 UI: HP, 방어막, 에너지, 손패, 적 의도 표시.
5. PlayMode 또는 EditMode 테스트로 전투 시작/카드 사용/적 턴 전환을 검증한다.
...
- `BattleManager`의 `Skill` 카드 처리는 현재 `statusEffect` 부여만 수행한다. `분노`, `도발` 같은 전사 스킬은 다음 단계에서 명시적 카드 효과 처리기로 연결해야 한다.
