...
namespace TheraBytes.BetterUi
{
    ...
    [ExecuteInEditMode]
#endif
    [HelpURL(RootDirectory.HelpUrl + "BetterContentSizeFitter.html")]
    ...
    public class BetterContentSizeFitter : ContentSizeFitter, IResolutionDependency, ILayoutChildDependency, ILayoutElement, ILayoutIgnorer
    {
        ...
        public FloatSizeModifier CurrentMaxHeight { get { return maxHeightSizers.GetCurrentItem(maxHeightSizerFallback); } }
        ...
        {
            Vector2 padding = CurrentPadding.CalculateSize(this, nameof(CurrentPadding));
        ...
        float ClampSize(RectTransform.Axis axis, float size)
        {
            switch (axis)
            {
                case RectTransform.Axis.Horizontal:
                    ...
                    {
                        size = Mathf.Max(size, CurrentMinWidth.CalculateSize(this, nameof(CurrentMinWidth)));
                    }

                    if (CurrentSettings.HasMaxWidth)
                    {
                        size = Mathf.Min(size, CurrentMaxWidth.CalculateSize(this, nameof(CurrentMaxWidth)));
                    }
                    break;
                ...
                case RectTransform.Axis.Vertical:
                    ...
                    {
                        size = Mathf.Max(size, CurrentMinHeight.CalculateSize(this, nameof(CurrentMinHeight)));
                    }

                    if (CurrentSettings.HasMaxHeight)
                    {
                        size = Mathf.Min(size, CurrentMaxHeight.CalculateSize(this, nameof(CurrentMaxHeight)));
                    }
                    break;
            }
