...
namespace TheraBytes.BetterUi
{
    ...
    [ExecuteInEditMode]
#endif
    [HelpURL(RootDirectory.HelpUrl + "BetterAxisAlignedLayoutGroup.html")]
        ...
        : HorizontalOrVerticalLayoutGroup, IBetterHorizontalOrVerticalLayoutGroup, IResolutionDependency
    {
        ...
        {
            // Note: We have to wait a frame as the child elements may not be ready yet.
            //       We don't call `SetDirty()` as this seems to fail in some situations
            //       and would require two frames to wait for some reason (Unity bug?).
            yield return null;
        ...
        void ApplySettings(Settings settings)
        {
            ...
            SetProperty(ref this.m_ChildAlignment, settings.ChildAlignment);
            SetProperty(ref this.m_ChildForceExpandWidth, settings.ChildForceExpandWidth);
            SetProperty(ref this.m_ChildForceExpandHeight, settings.ChildForceExpandHeight);

#if !(UNITY_5_4) && !(UNITY_5_3)
            SetProperty(ref this.m_ChildControlWidth, settings.ChildControlWidth);
            SetProperty(ref this.m_ChildControlHeight, settings.ChildControlHeight);
#endif
#if UNITY_2019_1_OR_NEWER
            SetProperty(ref this.m_ChildScaleWidth, settings.ChildScaleWidth);
            SetProperty(ref this.m_ChildScaleHeight, settings.ChildScaleHeight);
#endif
#if UNITY_2020_1_OR_NEWER
            SetProperty(ref this.m_ReverseArrangement, settings.ReverseArrangement);
#endif
        }
