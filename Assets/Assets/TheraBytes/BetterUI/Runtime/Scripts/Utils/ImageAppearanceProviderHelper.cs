...
namespace TheraBytes.BetterUi
{
    public static class ImageAppearanceProviderHelper
    {
            ...
            VertexMaterialData materialProperties)
        {
            ...
            vertexHelper.AddVert(new Vector3(posMin.x, posMin.y, 0f), colors[0], new Vector4(uvMin.x, uvMin.y),
                uv1, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMin.x, posMax.y, 0f), colors[1], new Vector4(uvMin.x, uvMax.y),
                uv1, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMax.x, posMax.y, 0f), colors[2], new Vector4(uvMax.x, uvMax.y),
                uv1, normal, tangent);
            vertexHelper.AddVert(new Vector3(posMax.x, posMin.y, 0f), colors[3], new Vector4(uvMax.x, uvMin.y),
                uv1, normal, tangent);
        ...
        public static Color32 GetColor(ColorMode mode, Color32 a, Color32 b, Rect bounds, float x, float y)
        {
            switch (mode)
            {
                ...
                case ColorMode.HorizontalGradient:
                {
                    ...
                    return Color32.Lerp(a, b, amount);
                }
